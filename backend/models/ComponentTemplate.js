...
class ComponentTemplate {
  static async list(type, parentId = null) {
    if (type === "fractile") {
      ...
      if (parentId) {
        const result = await pool.query(
          `SELECT ct.id, ct.fractile_id, ct.name, ct.description, ct.created_by, ct.created_at, ct.updated_at,
      ...
      if (parentId) {
        const result = await pool.query(
          `SELECT tt.id, tt.cell_id, tt.name, tt.description, tt.created_by, tt.created_at, tt.updated_at,
                  ct.name as cell_name, ft.name as fractile_name, ct.fractile_id
  ...
  static async create(type, data) {
    ...
    if (type === "fractile") {
      ...
      const query = `INSERT INTO cell_templates (fractile_id, name, description, created_by) 
             VALUES ($1, $2, $3, $4)
             RETURNING id, fractile_id, name, description, created_by, created_at, updated_at`;
      ...
      const query = `INSERT INTO tier_templates (cell_id, name, description, created_by) 
             VALUES ($1, $2, $3, $4)
             RETURNING id, cell_id, name, description, created_by, created_at, updated_at`;
      const result = await pool.query(query, [
        cell_id,
        name,
        description,
  ...
  static async update(type, id, data) {
    ...
    let returnColumns;
    if (type === "fractile") {
      table = "fractile_templates";
      returnColumns =
        "id, name, description, created_by, created_at, updated_at";
    } else if (type === "cell") {
      table = "cell_templates";
      returnColumns =
        "id, fractile_id, name, description, created_by, created_at, updated_at";
    } else if (type === "tier") {
      table = "tier_templates";
      returnColumns =
        "id, cell_id, name, description, created_by, created_at, updated_at";
    ...
    values.push(id);
    const query = `UPDATE ${table} SET ${fields.join(", ")} WHERE id = $${idx}
