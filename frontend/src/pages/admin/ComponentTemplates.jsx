...
const ComponentTemplates = () => {
  ...
  const addHierarchyCell = () => {
    setHierarchyCells([...hierarchyCells, { 
      id: nextCellId, 
      name: "", 
      description: "", 
      tiers: []
  ...
  const updateHierarchyCell = (cellId, field, value) => {
    setHierarchyCells(hierarchyCells.map(c => 
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (modalMode === "create") {
        ...
        await templateAPI.update(`${type}s`, id, { name: form.name, description: form.description });
  ...
  const handleDelete = async (type, id, parentId = null) => {
    const childType = type === "fractile" ? "cells and tiers" : type === "cell" ? "tiers" : "";
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button className="bg-sky-400 hover:bg-sky-500" onClick={() => navigate('/dashboard')}>
          Back to Dashboard
        </Button>
      </div>
      
      <p className="text-sm text-gray-600 mb-4">
        Hierarchy: Fractile → Cell → Tier. Click the arrow to expand and view nested items.
      </p>
                  ...
                  <TableRow className="bg-blue-50 hover:bg-blue-100 cursor-pointer" onClick={() => toggleFractile(fractile.id)}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-0.5 rounded">FRACTILE</span>
                      </div>
                    </TableCell>
                    <TableCell className="font-medium text-blue-800">{fractile.name}</TableCell>
                    <TableCell className="text-gray-600">{fractile.description || "-"}</TableCell>
                    <TableCell className="text-center">
                      {expandedFractiles[fractile.id] && cellsData[fractile.id] ? (
                        <span className="text-sm text-green-600 font-medium">{getCellCount(fractile.id)} Cells</span>
                      ) : (
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openEdit("fractile", fractile)}>
                          <FaEdit className="w-3 h-3 text-gray-600" />
                        </Button>
                        <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => handleDelete("fractile", fractile.id)}>
                  ...
                  {expandedFractiles[fractile.id] && (
                    <>
                      {loadingCells[fractile.id] ? (
                        <TableRow>
                          <TableCell colSpan={6} className="pl-12 text-gray-500 text-sm">Loading cells...</TableCell>
                        </TableRow>
                      ) : !cellsData[fractile.id] || cellsData[fractile.id].length === 0 ? (
                        <TableRow>
                        ...
                        cellsData[fractile.id].map((cell) => (
                          <React.Fragment key={cell.id}>
                            ...
                            <TableRow className="bg-green-50 hover:bg-green-100 cursor-pointer" onClick={() => toggleCell(cell.id)}>
                              <TableCell className="text-center pl-8">
                                <button className="p-1 hover:bg-green-200 rounded">
                                  {expandedCells[cell.id] ? (
                                    ...
                                    <FaChevronRight className="w-3 h-3 text-green-600" />
                                  )}
                                </button>
                              </TableCell>
                              <TableCell>
                                <div className="flex items-center gap-2 pl-4">
                                  <FaCube className="w-4 h-4 text-green-600" />
                                  <span className="text-xs font-semibold text-green-600 bg-green-100 px-2 py-0.5 rounded">CELL</span>
                                </div>
                              </TableCell>
                              <TableCell className="font-medium text-green-800">{cell.name}</TableCell>
                              <TableCell className="text-gray-600">{cell.description || "-"}</TableCell>
                              <TableCell className="text-center">
                                {expandedCells[cell.id] && tiersData[cell.id] ? (
                                  <span className="text-sm text-purple-600 font-medium">{getTierCount(cell.id)} Tiers</span>
                                ) : (
                                  <span className="text-sm text-gray-400">-</span>
                                )}
                              </TableCell>
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openEdit("cell", { ...cell, fractile_id: fractile.id })}>
                                    <FaEdit className="w-3 h-3 text-gray-600" />
                                  </Button>
                                  <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => handleDelete("cell", cell.id, fractile.id)}>
                                    <FaTrash className="w-3 h-3" />
                                  </Button>
                                </div>
                              </TableCell>
                            ...
                            {expandedCells[cell.id] && (
                              <>
                                {loadingTiers[cell.id] ? (
                                  <TableRow>
                                    <TableCell colSpan={6} className="pl-20 text-gray-500 text-sm">Loading tiers...</TableCell>
                                  </TableRow>
                                ) : !tiersData[cell.id] || tiersData[cell.id].length === 0 ? (
                                  <TableRow>
                                    ...
                                    <TableRow key={tier.id} className="bg-purple-50 hover:bg-purple-100">
                                      ...
                                      <TableCell>
                                        <div className="flex items-center gap-2 pl-8">
                                          ...
                                          <span className="text-xs font-semibold text-purple-600 bg-purple-100 px-2 py-0.5 rounded">TIER</span>
                                        </div>
                                      </TableCell>
                                      <TableCell className="font-medium text-purple-800">{tier.name}</TableCell>
                                      <TableCell className="text-gray-600">{tier.description || "-"}</TableCell>
                                      <TableCell></TableCell>
                                      <TableCell className="text-right">
                                        <div className="flex justify-end gap-1">
                                          <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openEdit("tier", { ...tier, cell_id: cell.id })}>
                                            <FaEdit className="w-3 h-3 text-gray-600" />
                                          </Button>
                                          <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => handleDelete("tier", tier.id, cell.id)}>
                                            <FaTrash className="w-3 h-3" />
                                          </Button>
                                        </div>
                                      </TableCell>
            ...
            <form onSubmit={handleSave} className="space-y-4 mt-4">
              ...
              <div className="border-2 border-blue-200 rounded-lg p-4 bg-blue-50">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-semibold text-blue-800">Fractile (Root)</span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFractileData({ ...fractileData, name: e.target.value })}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFractileData({ ...fractileData, description: e.target.value })}
                ...
                {hierarchyCells.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {hierarchyCells.map((cell, cellIndex) => (
                      <div key={cell.id} className="border border-green-200 rounded-lg p-3 bg-green-50">
                        <div className="flex items-center gap-2 mb-2">
                          <FaCube className="text-green-600 w-4 h-4" />
                          <span className="font-medium text-green-800 text-sm">Cell {cellIndex + 1}</span>
                          <button
                            type="button"
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          <Input
                            value={cell.name}
                            onChange={(e) => updateHierarchyCell(cell.id, "name", e.target.value)}
                            placeholder="Cell name"
                            className="bg-white text-sm"
                          />
                          <Input
                            value={cell.description}
                            onChange={(e) => updateHierarchyCell(cell.id, "description", e.target.value)}
                        ...
                        <div className="ml-4 border-l-2 border-gray-200 pl-3 mt-2">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-medium text-gray-500">Tiers</span>
                            <Button
                              type="button"
                              size="sm"
                              variant="ghost"
                          ...
                          {cell.tiers.length === 0 ? (
                            <div className="text-xs text-gray-400 italic py-1">
                              {cell.name.trim() ? 'No tiers yet.' : 'Enter cell name first.'}
                            </div>
                          ) : (
                            <div className="space-y-2">
                              {cell.tiers.map((tier, tierIndex) => (
                                <div key={tier.id} className="flex items-center gap-2 bg-purple-50 border border-purple-200 rounded p-2">
                                  <FaLayerGroup className="text-purple-600 w-3 h-3 flex-shrink-0" />
                                  <Input
                                    value={tier.name}
                                    onChange={(e) => updateHierarchyTier(cell.id, tier.id, "name", e.target.value)}
                                    placeholder={`Tier ${tierIndex + 1} name`}
                                    className="bg-white text-xs h-7 flex-1"
                                  />
                                  <Input
                                    value={tier.description}
                                    onChange={(e) => updateHierarchyTier(cell.id, tier.id, "description", e.target.value)}
                                    placeholder="Description"
                                    className="bg-white text-xs h-7 flex-1"
                                  />
                                  <button
                                    type="button"
                                    onClick={() => removeHierarchyTier(cell.id, tier.id)}
              ...
              {fractileData.name.trim() && (
                <div className="bg-gray-100 rounded-lg p-3 text-sm">
                  ...
                  <span className="text-gray-600">
                    ...
                    {hierarchyCells.reduce((sum, c) => sum + c.tiers.filter(t => t.name.trim()).length, 0) > 0 &&
